...
class ControllerOCTemplatesModuleOctPopupPurchase extends Controller {
		...
		if (!$is_google_bot && ($this->config->get('oct_popup_purchase_status') || $this->config->get('oct_popup_purchase_byoneclick_status')) && $this->config->get('config_checkout_guest') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && (isset($this->request->post['product_id']) && !empty($this->request->post['product_id']))) {
			...
			if ($this->validate()) {
				...
				if ($this->customer->isLogged()) {
					...
					$order_data['firstname'] = $order_data['shipping_firstname'] = $order_data['payment_firstname'] = isset($this->request->post['name']) ? $this->request->post['name'] : $this->language->get('heading_title');
	...
	protected function validate() {
		$oct_popup_purchase_data = $this->config->get('oct_popup_purchase_data');
		$oct_popup_purchase_byoneclick_data = $this->config->get('oct_popup_purchase_byoneclick_data');
		...
		if (isset($this->request->post['oct_byoneclick'])) {
			if (!isset($this->request->post['telephone']) || empty($this->request->post['telephone'])) {
				...
				$phone_count = utf8_strlen(str_replace(['_','-','(',')','+',' '], "", $oct_popup_purchase_byoneclick_data['mask']));

				if (utf8_strlen(str_replace(['_','-','(',')','+',' '], "", $this->request->post['telephone'])) < $phone_count || !preg_match('/[0-9,\-,+,\(,\),_]/', $this->request->post['telephone'])) {
