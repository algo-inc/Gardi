...
class ControllerCheckoutOctFastorder extends Controller {
    public function index() {
        ...
        $data['mask'] = $oct_fastorder_data['mask'] ?? '';
        $data['refresh_after_citychange'] = $oct_fastorder_data['refresh_after_citychange'] ?? '';
        $data['refresh_after_addnew_address'] = $oct_fastorder_data['refresh_after_addnew_address'] ?? '';
        $data['user_block_sort'] = (!empty($oct_fastorder_data['user_block_sort'])) ? (int)$oct_fastorder_data['user_block_sort'] : 2;
        $data['delivery_block_sort'] = (!empty($oct_fastorder_data['delivery_block_sort'])) ? (int)$oct_fastorder_data['delivery_block_sort'] : 1;
        $data['cart_block_sort'] = (!empty($oct_fastorder_data['cart_block_sort'])) ? (int)$oct_fastorder_data['cart_block_sort'] : 3;
        $data['show_payment_adres'] = $oct_fastorder_data['show_payment_adres'] ?? '';
        $data['show_country'] = $oct_fastorder_data['show_country'] ?? '';
        $data['show_cost_in_delivery_block'] = $oct_fastorder_data['show_cost_in_delivery_block'] ?? '';

        $this->document->addStyle('catalog/view/theme/oct_ultrastore/stylesheet/oct-fastorder.min.css');
    ...
    public function guest_validate() {
        ...
        if (!$json) {
            ...
            if ($this->cart->hasShipping() && ($this->session->data['guest']['shipping_address'] || $this->session->data['shipping_address'])) {
        ...
        if (!$json) {
            ...
            if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new') {
                ...
                if (!$json) {
                    ...
                    $this->session->data['shipping_address_id'] = $this->model_account_address->addAddress($this->customer->getId(), $shipping_address);
