...
class ControllerOCTemplatesFaqOCTProductFaq extends Controller {
    public function index($data = []) {
        ...
        $this->load->language('octemplates/oct_ultrastore');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ($data['product_id']) {
				...
				if ($product_info && !isset($json['error'])) {
					...
					$message .= sprintf($this->language->get('text_faq_email_body'), $product_info['name'], HTTP_SERVER . 'index.php?route=product/product&product_id=' . (int)$data['product_id']) . "\n\n";
