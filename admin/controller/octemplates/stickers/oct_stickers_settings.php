...
class ControllerOCTemplatesStickersOCTStickersSettings extends Controller {
	...
	public function index() {
		...
		$data['oct_stickers_data'] = [];

        if (isset($this->request->post['oct_stickers_data'])) {
			$oct_stickers_data = $this->request->post['oct_stickers_data'];
		} else {
			$oct_stickers_data = $this->config->get('oct_stickers_data');
		}

		$data['placeholder'] = $this->model_tool_image->resize('no-thumb.png', 50, 50);

		foreach ($oct_stickers_data as $key => $stickers) {
			foreach ($stickers as $k_s => $sticker) {
				$data['oct_stickers_data'][$key][$k_s] = $sticker;

				if (isset($sticker['image']) && !empty($sticker['image']) && file_exists(DIR_IMAGE . $sticker['image'])) {
					$data['oct_stickers_data'][$key][$k_s]['thumb'] = $this->model_tool_image->resize($sticker['image'], 50, 50);
				} else {
					$data['oct_stickers_data'][$key][$k_s]['thumb'] = $this->model_tool_image->resize('no-thumb.png', 50, 50);
				}
			}
		}
