...
class ModelOCTemplatesWidgetsOCTSeoGeneration extends Model {
	public function seoUrlGenerator($type, $language_id, $store_id, $data, $element_id) {
		$oct_seo_url_data = $this->config->get('theme_oct_ultrastore_seo_url_data');
		...
		if (!empty($oct_seo_url_data[$type])) {
			...
			switch ($type) {
				...
				case 'information':
					$name = (isset($data['information_description'][$language_id]['title']) && !empty($data['information_description'][$language_id]['title'])) ? strip_tags(html_entity_decode($data['information_description'][$language_id]['title'], ENT_QUOTES, 'UTF-8')) : '';
					$model = '';
					$sku = '';
					$brand = '';
					break;
			...
			if (!empty($name)) {
				...
				while ($unic === false) {
					...
					$sql = $this->db->query("SELECT seo_url_id FROM " . DB_PREFIX . "seo_url WHERE store_id = '" . (int)$store_id . "' AND keyword = '" . $this->db->escape($oct_keyword_translit.$t) . "'");
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET store_id = '" . (int)$store_id . "', language_id = '" . (int)$language_id . "', query = '". str_replace('_', '', $type) ."_id=" . (int)$element_id . "', keyword = '" . $this->db->escape($oct_keyword_translit.$t) . "'");
			}
		}
	}
