...
class ModelOCTemplatesWidgetsOctMinify extends Model {
	public function octMinifyCss($data) {
		$oct_ultrastore_data = isset($this->config->get('theme_oct_ultrastore_data')['minify']) ? 1 : 0;
		$oct_styles = $this->document->getOctStyles();

		if ($oct_ultrastore_data) {
			$oct_ultrastore_version = "?v=" . $this->config->get('theme_oct_ultrastore_version');
			...
			if (!file_exists($dir_minify . 'cache/' . $oct_name)) {
				...
				if (file_exists($dir_minify . 'cache/' . $oct_name) && filesize($dir_minify . 'cache/' . $oct_name)) {
					...
					if ($oct_content) {
						$oct_content = preg_replace('!/\*[^*]*\*+([^/][^*]*\*+)*/!', '', $oct_content);
						$oct_content = str_replace('m+', 'm + ', $oct_content);
			...
			if (file_exists($dir_minify . 'cache/' . $oct_name)) {
				...
				$oct_styles[0] = [
					//'href' => 'min/cache/' . $oct_name . $oct_ultrastore_version,
	...
	public function octMinifyJs() {
		...
		if ($oct_ultrastore_data) {
			...
			if (file_exists($dir_minify . 'cache/' . $oct_name)) {
				...
				//$oct_scripts[0] = 'min/cache/' . $oct_name . $oct_ultrastore_version;
