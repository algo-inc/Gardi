...
class ControllerOCTemplatesModuleOctLiveSearch extends Controller {
	public function index() {
		if ($this->config->get('theme_oct_ultrastore_live_search_status') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	        ...
	        if (isset($this->request->post['key']) && !empty($this->request->post['key'])) {
		        $oct_live_search_data = $this->config->get('theme_oct_ultrastore_live_search_data');
